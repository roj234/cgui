...
(参考图显示效果差主要是我手机垃圾, 当然也可能便宜TN屏在手机和眼睛看来差异比较大)
...
*   **网页级质量**：利用 Chrome 的字体引擎生成带抗锯齿的 Alpha Mask。单片机上的数字也能拥有 Google Fonts 的圆润感。
*   **零内存开销**：不维护复杂的组件树或上下文栈。所有的 UI 状态都被编译为单层 C 语言结构体，RAM 占用仅取决于你的业务数据。
*   **极致性能**：真正的细粒度增量更新。基于字符变更检测，只向 LCD 推送发生变化的单个字符。
*   **低端友好**：使用魔改的QOI格式对图像进行压缩，占用空间只有传统位图的 10-20%，`STM32F103C8T6` 也能上桌吃饭 

## ⚙ 工作流程

1.  **设计**：使用任意 HTML/CSS 编写 UI 稿。通过 `.cg-` CSS类标记动态数据，通过 `cg-alphabet` 指定字符集……
2.  **编译**：`npm run build`。脚本启动无头浏览器（_是的，如果你想可以使用火狐_）渲染页面，自动计算所有元素的分辨率、坐标、并提取位图。
3.  **引用**：脚本生成 `gui_XXX.h`。包含 QOI 压缩的背景切片、字体掩模、以及自动生成的增量更新逻辑 `CG_xxx_Update()`。
4.  **部署**：驱动层最少仅需提供 `CGUI_Lcd_SetRect` `CGUI_Lcd_WriteColor` 函数，可选DMA刷新支持（`WriteRepeat`/`WriteArray`）。

## 🌟 特性

*   **字体引擎**：自动提取特定字符集，支持非等宽字体（WIP），支持 Alpha 掩模（WIP）或彩色位图。
...
* 哥们，你都用Chrome设计界面了，不会还在用 NodeJS 8 吧
```bash
npm run build
```
将会为每一个具有 `id` 的 `.cg-screen` 生成 `output/gui_<id>.h` 和 `output/gui_<id>.c`
...
## 📊 性能表现 (160x128 屏幕示例)
*   **静态内存 (RAM)**: 至少256字节的栈用于解压缩 + 数十字节结构体 (上一次的状态)。
*   **代码量 (Flash)**: 4KB固件代码 + 取决于位图数量，一个界面可能十几KB。
...
**CGUI - 让我们在 90 年代的硬件 (是的，我只有64.0KB内存) 上渲染 2026 年的设计，这正是 2026 年嵌入式开发该有的样子。**